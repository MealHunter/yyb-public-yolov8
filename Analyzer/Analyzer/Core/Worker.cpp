                ...
                if (mControl->videoIndex > -1) {
                    ...
                    if (ret == 0) {
                        ...
                        if (ret == 0) {
                            ...
                            // 定义一个存储所有识别到物品的 vector
                            std::set<std::string> objects;

                            //绘制start
                            cv::polylines(image, mControl->recognitionRegion_points, mControl->recognitionRegion_points.size(), cv::Scalar(0, 0, 255), frame, 8);//绘制多边形
                            if (happenDetects.size() > 0) {
                                ...
                                for (size_t i = 0; i < happenDetects.size(); i++)
                                {
                                    ...
                                    std::map<std::string, std::string> translationMap = {
                                        ...
                                        {"bird", "鸟"}, {"cat", "猫"}, {"dog", "狗"}, {"horse", "马"}, {"sheep", "绵羊"},
                                        ...
                                        {"dining table", "餐桌"}, {"toilet", "卫生间"}, {"tv", "电视"}, {"laptop", "笔记本电脑"},
                                    ...
                                    // 输入要替换的英文单词
                                    std::string input = happenDetects[i].class_name;

                                    // 查找并替换为中文
                                    if (translationMap.find(input) != translationMap.end()) {
                                        std::cout << "中文翻译: " << translationMap[input] << std::endl;
                                    }
                                    else {
                                        std::cout << "未找到翻译: " << input << std::endl;
                                    }

                                    objects.insert(translationMap[input]);
                                    ...
                                    cv::rectangle(image, cv::Rect(x1, y1, (x2 - x1), (y2 - y1)), cv::Scalar(0, 0, 255), frame, cv::LINE_8, 0);
                                    ...
                                    //object << class_name;
                                    //std::string count_title = "场景识别:" + object.str();
                                    //cv::rectangle(image, cv::Point(20 * (image.cols / 1920.0), 180 * (image.rows / 1280.0)), cv::Point(400 * (image.cols / 1920.0), 250 * (image.rows / 1280.0)), cv::Scalar(25, 25, 25), cv::FILLED);
                                    //putTextHusky(image, count_title.c_str(), Point(20 * (image.cols / 1920.0), 200 * (image.rows / 1280.0)), Scalar(255, 255, 255), 30, "楷体", true, true);
                            ...
                            std::string count_title = "人员识别:" + count_stream.str() + "人";
                            cv::rectangle(image, cv::Point(20 * (image.cols / 1920.0), 100 * (image.rows / 1280.0)),cv::Point(300 * (image.cols / 1920.0), 180 * (image.rows / 1280.0)), cv::Scalar(50, 50, 50), cv::FILLED);
                            putTextHusky(image, count_title.c_str(), Point(20 * (image.cols / 1920.0), 120 * (image.rows / 1280.0)), Scalar(255, 255, 255), baseFat, "楷体", true, true);
                            

                            // 拼接整个列表的内容到 count_title
                            std::stringstream objectStream;
                            for (const auto& obj : objects) {
                                objectStream << obj << ",";  // 将每个目标名加入到 stream 中，用空格分隔
                            }
                            std::string scene = "场景识别:" + objectStream.str();
                            cv::rectangle(image, cv::Point(20 * (image.cols / 1920.0), 180 * (image.rows / 1280.0)), cv::Point(400 * (image.cols / 1920.0), 250 * (image.rows / 1280.0)), cv::Scalar(50, 50, 50), cv::FILLED);
                            putTextHusky(image, scene.c_str(), Point(20 * (image.cols / 1920.0), 200 * (image.rows / 1280.0)), Scalar(255, 255, 255), baseFat, "楷体", true, true);
                            //绘制end

                            //mwebsocket->main(mControl->count);
                            // 更新共享数据文件
